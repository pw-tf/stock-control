...
**Card artwork**: client cards carry the manufacturer wordmark in a 72×44 landscape slot; device cards carry a photo of the terminal standing upright in a 48×64 portrait slot (64×40 and 44×58 below 600px). Two shapes because the artwork is two shapes, and the two never appear on the same screen. Files are found by convention rather than a lookup table, so adding artwork is a filename and never a code change:
...
- Anything missing falls back to a lettered chip, so the vendor cards show chips today; dropping `cba.webp` into `assets/logos/` gives that card a logo with no other change
...
- No bank logos are shipped. They are not in any package registry, and this environment's egress policy reaches registries only, so they cannot be fetched here — the lookup above is the drop-in point when the files are to hand. Two real logos beside three chips would be worse than five consistent chips, so the chips stay uniform until the whole set exists
...
- The lettered chip fallback takes the shape of whichever slot it stands in, so a device row mixing artwork and fallbacks still has one card height
...
- Source artwork is normalised by `tools/process-guide-art.mjs` (a dev utility needing `npm i sharp`; the app itself stays dependency-free). The pipeline is **measure → level → mirror → repair → stand upright**: the angle comes from a PCA of the alpha mask rather than from eye, and levelling first is what lets the Move5000 branding repair be specified in fixed pixel coordinates, since those are measured against the horizontal pose and survive any change to how the device is finally presented. Everything is then trimmed, scaled to a common content box and centred on one 200×320 canvas so no card looks heavier than its neighbour. Re-run it when new artwork arrives
